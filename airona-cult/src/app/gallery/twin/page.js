...
export default function TwinGallery() {
  ...
  const fetchTwinData = async (pageIndex, pageSize) => {
    const response = await fetch(`/api/content/twin?page=${pageIndex}&pageSize=${pageSize}`);
    if (!response.ok) {
  ...
  const loadMore = async (pageIndex) => {
    ...
    try {
      const newItems = await fetchTwinData(pageIndex, PAGE_SIZE);
  ...
  useEffect(() => {
    const inititalLoad = async () => {
      setLoading(true);
      try {
        const newItems = await fetchTwinData(0, PAGE_SIZE);
        if (newItems.length > 0) {
          setItems(newItems);
          setPage(0);
        }
